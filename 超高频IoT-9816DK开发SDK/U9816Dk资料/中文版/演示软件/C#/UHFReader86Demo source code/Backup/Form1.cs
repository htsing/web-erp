...
namespace UHFReader86Demo
{
    public partial class Form1 : Form
    {
        ...
        {
            if (text_target.Text.Trim() =="") return;
        ...
        {
            byte powerDbm = Convert.ToByte(ComboBox_PowerDbm.Text);
            fCmdRet = RWDev.SetRfPower(ref fComAdr, powerDbm, frmcomportindex);
            if (fCmdRet != 0)
            {
                string strLog = "设置功率失败，原因： " + GetReturnCodeDesc(fCmdRet);
        ...
        private void CheckBox_SameFre_Click(object sender, EventArgs e)
        {
