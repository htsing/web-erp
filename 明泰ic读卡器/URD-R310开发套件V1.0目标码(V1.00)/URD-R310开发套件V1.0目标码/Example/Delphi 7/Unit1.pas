...
begin
  ...
  wdata[8]:=$0;
listbox1.Items.clear;
st:=cpu_reset(icdev,data[0]);                   //∏¥Œª£ª
if st<>0 then
  begin
  ...
  for i:=0 to 7 do
    wdata[8]:=wdata[i] XOR Wdata[8];     //º∆À„BCC
...
if st<>0 then
       begin
         listbox1.items.Add('chk_1604 error');
         exit;
       end
     else
       listbox1.items.add('chk_1604 OK');
st:=srd_1604(icdev,10,32,15,data2);
if st<>0 then
  begin
    listbox1.items.add('srd_1604 error');
    exit;
  end
else
  listbox1.items.add('srd_1604 OK');
end;
...
if st<>0 then
       begin
         listbox1.items.Add('chk_24c01a error');
         exit;
       end
     else
       listbox1.items.add('chk_24c01a OK');
st:=srd_24c01a(icdev,32,15,data2);
if st<>0 then
  begin
    listbox1.items.add('srd_24c01a error');
    exit;
  end
else
  listbox1.items.add('srd_24c01a OK');
end;
